...
from utilss.indent_utils import match_indent, match_indent_by_first_line, WorkspaceManager
...
from EvenInfo.event import EventType, RealtimeEvent
...
# Extend AgentImplOutput to add success property
class ExtendedAgentImplOutput(AgentImplOutput):
    @property
...
def if_truncate_needed(content: str, truncate_after: int | None = MAX_RESPONSE_LEN):
...
async def run(
    ...
    try:
        ...
        return (
            ...
            if_truncate_needed(stdout.decode(), truncate_after=truncate_after),
            if_truncate_needed(stderr.decode(), truncate_after=truncate_after),
        )
...
class StrReplaceEditorTool(AgentPlugin):
...
"""
    ...
    def __init__(
        ...
        ignore_indentation_for_str_replace: bool = True,
    ...
    async def run_impl(
        ...
        try:
            ...
            if not is_path_in_directory(self.workspace_manager.root, _ws_path):
                ...
                return ExtendedAgentImplOutput(
    ...
    def undo_edit(self, path: Path) -> ExtendedAgentImplOutput:
    ...
    def _make_output(
        ...
        file_content = if_truncate_needed(file_content)
        if self.expand_tabs:
