...
from deepsearcher.reasoning.builders.report import ReportBuilder
from deepsearcher.reasoning.tools.tool_history import ToolHistory
from deepsearcher.reasoning.clients.openai_client import OpenAIClient
from deepsearcher.reasoning.config import get_config, update_config
from deepsearcher.reasoning.models.action import Action
from deepsearcher.reasoning.models.output import ModelOutput
from deepsearcher.reasoning.models.trace import Trace, Turn
from deepsearcher.reasoning.tools.registry import (
    format_tool_descriptions,
    get_all_tools,
    get_tool,
)
from deepsearcher.reasoning.builders.report import ReportType
...
class ReasoningAgent:
    ...
    def __init__(
        ...
        max_turns: int = 40,  # Add configurable turn limit
        timeout_seconds: int = 220,  # Add configurable timeout (3 minutes)
    ):
        ...
        Args:
            ...
            timeout_seconds: Maximum time to run before forcing completion
        """
    ...
    async def force_completion(self, reason: str, on_token: Optional[Callable[[str], None]] = None, is_stream: bool = False) -> str:
        """Force completion when limits are reached."""
        logging.warning(f"Forcing completion due to: {reason}")
        ...
        try:
            ...
            final_report = f"**Note: Research was completed due to {reason}**\n\n{final_report}"
            ...
            logging.error("Error in forced completion: %s", str(e))
            return f"Research incomplete due to {reason}. Error generating final report: {str(e)}"

    async def run(self, on_token: Optional[Callable[[str], None]] = None, is_stream: bool = False) -> str:
        """Run the agent with safety limits."""
        turn = 0
        ...
        while True:
            ...
            if turn >= self.max_turns:
                logging.warning(f"Reached maximum turns limit: {self.max_turns}")
                return await self.force_completion(f"maximum turn limit ({self.max_turns}) reached", on_token, is_stream)
            
            if time.time() - start_time > self.timeout_seconds:
                logging.warning(f"Reached timeout limit: {self.timeout_seconds} seconds")
                return await self.force_completion(f"timeout limit ({self.timeout_seconds}s) reached", on_token, is_stream)

            logging.info(f"Starting turn {turn + 1}/{self.max_turns}")
            ...
            try:
                ...
                return await self.force_completion(f"completion generation error: {str(e)}", on_token, is_stream)
            ...
            try:
                ...
                # Create a model output with no action to trigger completion
                model_output = ModelOutput(raw=content, action=None, is_last=True)
            ...
            if model_output.action:
                ...
                try:
                    ...
                    logging.info(f"Research completed successfully in {turn} turns")
                    return final_report

                except (asyncio.TimeoutError, asyncio.CancelledError) as e:
                    logging.error("Error generating report: %s", str(e))
                    return await self.force_completion(f"report generation error: {str(e)}", on_token, is_stream)

            # Add a small delay between turns to prevent rapid loops
